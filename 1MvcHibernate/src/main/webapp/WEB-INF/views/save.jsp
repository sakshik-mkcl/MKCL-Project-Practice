...
<html lang="en">
  <head>
    ...
    function validate(){
        ...
        if(id.value.trim() =="")
        {
            alert("Candidate Id should not be blanked.");
            id.style.border="dotted 3px red";
            return false;
         }
         else if(name.value.trim() =="")
        {
            alert("Candidate name should not be blanked.");
            name.style.border="dotted 3px red";
            return false;
         }
         else if(username.value.trim() =="")
        {
            alert("Candidate UserName should not be blanked.");
            username.style.border="dotted 3px red";
            return false;
         }
         else if (!(/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(username.value)))
         {
            alert("username is not in proper format");
            return false;
         }
         else if(password.value.trim() =="")
        {
            alert("Candidate Password should not be blanked.");
            password.style.border="dotted 3px red";
            return false;
         }
         else if(password.value.trim().length<5)
        {
            alert("Candidate Password is too short.");
            return false;
         }
  ...
  <body>
   ...
   <form onsubmit="return validate()" action="handleCandidate" method="post">
...
  <label for="female">Female</label><br>
 ...
 <div>
  ...
  <a href="/1MvcHibernate/addCandidate" class="btn btn-secondary">Reset</a>
